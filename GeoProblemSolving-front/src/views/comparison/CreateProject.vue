    ...
    createProject(name){
      this.$refs[name].validate(valid=>{
        if(valid){
          ...
          this.$api.cmp_project.create(projectInfo)
            .then(res=>{
              ...
              if(this.projectType==1){
                this.$router.push({path:`/cmp-project/comprihensive/${res.projectId}`})
              }else{
                this.$router.push({path:`/cmp-project/specific/${res.projectId}`})
              }
