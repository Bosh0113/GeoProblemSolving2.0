  ...
  <div>
    ...
    <Modal v-model="shareToolModal" width="800" title="Share tool">
      <Form>
        ...
        <FormItem v-if="formItem.select == 'selectProject'">
          <Select v-model="formItem.sharedProjectId" placeholder="Select Project">
            <Option v-for="item in userProject" :value="item.id" :key="item.id">{{ item.name }}</Option>
          </Select>
        </FormItem>
      ...
      <div slot="footer">
        ...
        <Button type="success">Share</Button>
      </div>
    </Modal>
    ...
    methods: {
      initTool: function(){
        ...
        if (this.$store.getters.userInfo.createdProjects != null){
          //需要修改，这里面只是存了 project 的id
          let userCreatedProject = this.$store.getters.userInfo.createdProjects;
          for (let i=0; i<userCreatedProject.length; i++){
            project.aid = userCreatedProject[i].aid;
            project.name = userCreatedProject[i].name;
            userProjectList.push(project)
          }
        }
        if (this.$store.getters.userInfo.joinedProjects != null){
          let userManagedProject = this.$store.getters.userInfo.joinedProjects;
          for (let i=0; i<userManagedProject.length; i++){
            project.aid = userManagedProject[i].aid;
            project.name = userManagedProject[i].name;
            userProjectList.push(project)
          }
        }
        this.$set(this, "userProject", userProjectList);
