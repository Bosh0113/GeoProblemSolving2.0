...
Sidebar.prototype.init = function () {
   ...
   */
  {
    ...
    generalTab.onclick=function(){
      ...
      document.getElementById("model_tab").style.backgroundColor= "#f1f3f4";
      document.getElementById("general_tab").style.backgroundColor= "#ffffff";
      document.getElementById("general_tab").style.borderBottom = "0px";
      document.getElementById("model_tab").style.borderBottom = "1px solid #dadce0";
    };
    ...
    modelTab.onclick=function(){
      ...
      document.getElementById("general_tab").style.backgroundColor= "#f1f3f4";
      document.getElementById("model_tab").style.backgroundColor= "#ffffff";
      document.getElementById("model_tab").style.borderBottom = "0px";
    ...
    this.sideModelContainer.style.display = "none";
    this.sideModelContainer.id = "model";
    this.container.appendChild(this.sideModelContainer);

    this.sideGeneralContainer = document.createElement("div");
    this.sideGeneralContainer.style.height = "100%";
    this.sideGeneralContainer.id = "general";
    this.container.appendChild(this.sideGeneralContainer);
  }
  ...
  this.addGeneralPalette2('flowchart', 'Flowchart', dir + '/logical.xml',
 ...
 */
Sidebar.prototype.thumbWidth = 210;

/**
 * Specifies the height of the thumbnails.
 */
Sidebar.prototype.thumbHeight = 60;
...
Sidebar.prototype.addEntry = function (tags, fn) {
  if (this.taglist != null && tags != null && tags.length > 0) {
    ...
    var tmp = tags.toLowerCase().replace(/[\/\,\-\_\(\)]/g, ' ').split(' ');
...
Sidebar.prototype.addSearchPalette = function (expand) {
  ...
  this.sideModelContainer.appendChild(outer);
...
Sidebar.prototype.createItem = function (cells, title, showLabel, showTitle, width, height, allowCellsInserted) {
  ...
  if (!mxClient.IS_IOS) {
    mxEvent.addGestureListeners(elt, null, mxUtils.bind(this, function (evt) {
      if (mxEvent.isMouseEvent(evt) && title != null) {
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createVertexTemplateEntry = function (style, width, height, value, title, showLabel, showTitle, tags, model) {
  tags = (tags != null && tags.length > 0) ? tags : title.toLowerCase();

  return this.addEntry(tags, mxUtils.bind(this, function () {
    return this.createStateVertexTemplate(style, width, height, value, title, showLabel, showTitle, true, model);
  }));
};
 ...
 * 创建state和event的cell
 */
Sidebar.prototype.createStateVertexTemplate = function (style, width, height, value, title, showLabel, showTitle, allowCellsInserted, model) {
  var cells = [new mxCell((value != null) ? value : '', new mxGeometry(0, 0, width, height), style)];
  cells[0].vertex = true;
  if (model != undefined){
 ...
 * 为SideBar形状图例
 */
Sidebar.prototype.addGeneralPalette2 = function (id, title, stencilFile, style, ignore, onInit, scale, tags) {
  ...
  if (this.addStencilsToIndex)
  {
    ...
    mxStencilRegistry.loadStencilSet(stencilFile, mxUtils.bind(this, function(packageName, stencilName, displayName, w, h)
    {
      if (ignore == null || mxUtils.indexOf(ignore, stencilName) < 0)
      {
        ...
        fns.push(this.createVertexTemplateEntry('shape=' + packageName + stencilName.toLowerCase() + style,
            Math.round(w * scale), Math.round(h * scale), label, stencilName.replace(/_/g, ' '), null, null,
            this.filterTags(tmp.join(' '))));
      }
    }), false, false);

    this.addPaletteFunctions(id, title, true, fns, this.sideGeneralContainer);
  }
  else
  {
    this.addPalette(id, title, false, mxUtils.bind(this, function(content)
