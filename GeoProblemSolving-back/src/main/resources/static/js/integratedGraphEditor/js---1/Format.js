 ...
 * 新增 EventPanel 面板
 */
EventPanel = function(format,editorUi,container){
    BaseFormatPanel.call(this, format, editorUi, container);
    this.init();
};

mxUtils.extend(EventPanel, BaseFormatPanel);

/**
 * Adds the label menu items to the given menu and parent.
 */
EventPanel.prototype.init = function()
{
    var graph = this.editorUi.editor.graph;

    var model = graph.getSelectionModel().cells[0];
    var modelOid = graph.getSelectionModel().cells[0].mid;
    // var inputData = [];
    // var outputData = [];
    // var i;
    // for (i=0; i<computableModelList.size; i++){
    //     if(modelOid == computableModelList.content[i].oid){
    ...
    //         if(modelOid == hasSearchedTermsComputableModel[j].modelOid){
    ...
    this.container.appendChild(this.addinputData(this.createPanel(),model.inputData,model));
    this.container.appendChild(this.addoutputData(this.createPanel(),model.outputData,model));
};

EventPanel.prototype.addinputData = function(div,inputData,model){
    var ui = this.editorUi;
...
EventPanel.prototype.addoutputData = function(div,outputData,model){
 ...
 * 张硕
 * 2019.11.28
 * 新增  InputEventPanel 和 OutputEventPanel 面板
 */
InputEventPanel = function(format,editorUi,container){
    BaseFormatPanel.call(this, format, editorUi, container);
    this.init();
};

mxUtils.extend(InputEventPanel, BaseFormatPanel);


InputEventPanel.prototype.init = function()
{
    var graph = this.editorUi.editor.graph;
...
InputEventPanel.prototype.addInputEventInfo = function(div,event,modelName){
    ...
    var title = this.createTitle("Input Event Info : ");
    title.style.paddingBottom = '6px';
    title.style.fontSize = "14px";
    title.style.cursor = "default";
    div.appendChild(title);
 ...
 * 新增  InputEventPanel 和 OutputEventPanel 面板
 */
OutputEventPanel = function(format,editorUi,container){
    BaseFormatPanel.call(this, format, editorUi, container);
    this.init();
};

mxUtils.extend(OutputEventPanel, BaseFormatPanel);


OutputEventPanel.prototype.init = function()
{
	var graph = this.editorUi.editor.graph;
...
OutputEventPanel.prototype.addOutputEventInfo = function(div,event,modelName){
    ...
    var title = this.createTitle("Output Event Info : ");
    title.style.paddingBottom = '6px';
    title.style.fontSize = "14px";
    title.style.cursor = "default";
    div.appendChild(title);
