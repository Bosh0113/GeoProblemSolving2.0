...
Sidebar.prototype.init = function()
{
    ...
    if((url.indexOf("index")<0)&&(url.indexOf("logical")<0)){
        this.createConceptualSidebar(dir);
    }else{
        if(url.indexOf("index")>=0){
            this.createConceptualSidebar(dir);
        }else if(url.indexOf("logical")>=0){
			this.createLogicalSidebar(dir);
        }else{
...
Sidebar.prototype.createConceptualSidebar = function (dir) {
...
Sidebar.prototype.createLogicalSidebar = function (dir) {
	var titles = [
		"Start","End","Model-Item",
		"Data-Item","Condition-Item","Operation-Item"
	];

	var items = [
		"start","end","modelItem",
		"dataItem","conditionItem","operationItem"
	];

	this.addLogicalIconPalette(titles,items,dir+"/logical/",".png")
};

Sidebar.prototype.createComputableSidebar = function(dir){
...
{
	var tabContents = this.palettes['tabContent'].childNodes;
	var elt = document.createElement('div');
...
Sidebar.prototype.createItem = function(cells, title, showLabel, showTitle, width, height, allowCellsInserted)
{
    ...
    if(url.indexOf("logical")>=0){
        elt.style.width = 100 + 'px';
        elt.style.height = 55 + 'px';
    ...
    if(url.indexOf("logical")>=0){
        this.createThumb(cells, 100, 50, elt, title, showLabel, showTitle, width, height);
	}else{
...
Sidebar.prototype.addPaletteFunctions2 = function(id, title, expanded, fns)
{
    this.addPalette2(id, title, expanded, mxUtils.bind(this, function(content)
    {
        for (var i = 0; i < fns.length; i++)
        {
            content.appendChild(fns[i](content));
...
Sidebar.prototype.addPalette = function(id, title, expanded, onInit)
{
    ...
    tabContents[1].appendChild(outer);
...
Sidebar.prototype.addPalette2 = function(id, title, expanded, onInit)
{
    var tabContents = this.palettes['tabContent'].childNodes;
...
/**
 * Adds the Logical palette
 * */
Sidebar.prototype.addLogicalIconPalette = function(titles,items,prefix,postfix)
{
	var div = document.createElement("div");

	var title = this.createTitle("Basic Element");
	...
	iconUl.style.padding = '0';
	iconUl.style.margin = 0;
	div.appendChild(iconUl);
	...
	for(var i=0;i<items.length;i++){
		...
		var img = this.createVertexTemplateEntry('image;html=1;labelBackgroundColor=#ffffff;image=' + prefix + items[i] + postfix,112,67,'',titles[i],titles[i]!=null,null,null)(img);
		/*var img = document.createElement("img");
		img.src = prefix+items[i]+postfix;
		img.setAttribute("title",titles[i]);*/

		li.appendChild(img);

		iconUl.appendChild(li);
	}
    var title2 = this.createTitle("Related Info");
	title2.removeAttribute('title');
    title2.style.textAlign = 'center';
    title2.style.fontSize = '18px';
    div.appendChild(title2);
    this.container.appendChild(div);
    this.palettes['logical'] = div;
};