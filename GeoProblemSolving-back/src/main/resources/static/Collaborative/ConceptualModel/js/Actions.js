...
Actions.prototype.init = function()
{
    ...
    this.addAction('UpdateTask', function()
    {
    	if(window.taskID==null||window.taskID==="undefined"||window.taskID===""){
            ...
            dataJSON["taskId"]=window.taskID;
            dataJSON["graphXML"]=graphContentXML;
			dataJSON["conceptualXML"]=ConceptualScene;
            dataJSON["taskName"]=window.taskName;
            dataJSON["description"]=window.taskDescription;
            $.ajax({
                type: "POST",
                url: "/modelBuilder/conceptual/update",
                data: dataJSON,
                dataType: "text",
	...
	function deleteCellConcept(node){
		if(node.children!=null){
			for(var i=0;i<node.children.length;i++){
				if(node.children[i].concept!=undefined){
					var index;
					for(var j=0;j<graph.conceptItemList.length;j++){
						if(graph.conceptItemList[j].id==node.children[i].concept){
							index=j;
							break;
						}
					}
				...
				if(node.children[i].style==="group"){
					deleteCellConcept(node.children[i]);
				}
			}
		}
	}
    ...
    function deleteCells(includeEdges)
	{
		...
		if (cells != null && cells.length > 0)
		{
			...
			for(var i=0;i<cells.length;i++){
				if(cells[i].concept!=undefined){
					var index;
					for(var j=0;j<graph.conceptItemList.length;j++){
						if(graph.conceptItemList[j].id==cells[i].concept){
							index=j;
							break;
						}
					}
				...
				if(cells[i].style==="group"){
					deleteCellConcept(cells[i]);
				}
			}
	...
	this.addAction('ungroup', function()
	{
		...
		if(groupCell.concept!=undefined){
			var index;
				for(var j=0;j<graph.conceptItemList.length;j++){
					if(graph.conceptItemList[j].id==groupCell.concept){
						index=j;
						break;
					}
				}
