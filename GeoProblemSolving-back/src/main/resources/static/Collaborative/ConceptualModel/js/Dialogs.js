...
var ConceptTasksSaveDialog = function (editorUi) {
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function () {
        ...
        var conceptualContent = {
            ...
            conceptualXML: ConceptualScene
        }
        var conceptStr = JSON.stringify(conceptualContent);
        var blob = new Blob([conceptStr], {type: "text/plain;charset=utf-8"});  // type:可以设置别的文件类型
        var fileName = $('#graphNameSave').val() + ".cptm";
        var blobFile = new File([blob], fileName);
        var description = $('#graphDescriptionSave').val();
        var toolInfo = {
            toolName: "Conceptual modeling",
            toolUrl: "/GeoProblemSolving/Collaborative/ConceptualModel/index.html"
        };

        var formData = new FormData();
        formData.append("file", blobFile);
        formData.append("description", description);
        formData.append("type", "toolData: Conceptual_modeling");
