...
var ConceptTasksReadDialog=function(editorUi){
    ...
    $.ajax({
        type: "POST",
        url: "/modelBuilder/conceptual/read"+"?collaborativeId="+groupID,
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
    	if ($('input[name="tasks"]:checked').val()){
			...
			editorUi.toolbar.editorUi.editor.graph.setConceptItemList(taskData[$('input[name="tasks"]:checked').val()]["conceptualXML"]);
...
var ConceptTasksSaveDialog=function(editorUi){
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
        ...
        $.ajax({
            type: "POST",
            url: "/modelBuilder/conceptual/save",
			data: dataJSON,
			dataType:"text",
            success: function (data) {
