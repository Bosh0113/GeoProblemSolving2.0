...
/**
 * Get the XML of computational scene list in type string.
 */
Graph.prototype.getComputationalSceneXMLStr=function(){
	function createUUID () {
	...
	var computationalSceneXML=XMLDoc.createElement("ComputationalScene");
	var id=createUUID();
	computationalSceneXML.setAttribute("uid",id);
	computationalSceneXML.setAttribute("name","");
	computationalSceneXML.setAttribute("version","1");

	var modelInstancesXML=XMLDoc.createElement("ModelInstanceCollection");
	var dataInstancesXML=XMLDoc.createElement("DataInstanceCollection");
	var operationInstancesXML=XMLDoc.createElement("OperationInstanceCollection");
	...
	var modelInstances=this.computationalScene.modelInstances;
	var dataInstances=this.computationalScene.dataInstances;
	var operationInstances=this.computationalScene.operationInstances;

	for(var i=0;i<modelInstances.length;i++){
		var modelInstanceXML=XMLDoc.createElement("ModelInstance");
		modelInstanceXML.setAttribute("id",modelInstances[i].uid);
		for(var j=0;j<modelItems.length;j++){
			if(modelItems[j].uid==modelInstances[i].uid){
				modelInstanceXML.setAttribute("name",modelItems[j].name);
				modelInstanceXML.setAttribute("description",modelItems[j].description);
				break;
			}
		}
		var requestXML=XMLDoc.createElement("Request");
		requestXML.setAttribute("baseUrl",modelInstances[i].request.url);
		var parameterXML=XMLDoc.createElement("Parameter");
		parameterXML.setAttribute("key","Id");
		parameterXML.setAttribute("value",modelInstances[i].request.modelId);
		parameterXML.setAttribute("description",modelInstances[i].request.idDesc);
		requestXML.appendChild(parameterXML);
		modelInstanceXML.appendChild(requestXML);
		var statesXML=XMLDoc.createElement("States");
		for(var j=0;j<modelInstances[i].states.length;j++){
			var stateXML=XMLDoc.createElement("State");
			stateXML.setAttribute("id",modelInstances[i].states[j].id);
			stateXML.setAttribute("name",modelInstances[i].states[j].name);
			for(var k=0;k<modelInstances[i].states[j].events.length;k++){
				var eventXML=XMLDoc.createElement("Event");
				eventXML.setAttribute("name",modelInstances[i].states[j].events[k].name);
				eventXML.setAttribute("type",modelInstances[i].states[j].events[k].type);
				eventXML.setAttribute("dataInstance",modelInstances[i].states[j].events[k].dataId);
				stateXML.appendChild(eventXML);
			}
			statesXML.appendChild(stateXML);
		}
		modelInstanceXML.appendChild(statesXML);
		modelInstancesXML.appendChild(modelInstanceXML);
	}
	computationalSceneXML.appendChild(modelInstancesXML);

	for(var i=0;i<dataInstances.length;i++){
		var dataInstanceXML=XMLDoc.createElement("DataInstance");
		dataInstanceXML.setAttribute("id",dataInstances[i].uid);
		for(var j=0;j<dataItems.length;j++){
			if(dataItems[j].uid==dataInstances[i].uid){
				dataInstanceXML.setAttribute("name",dataItems[j].name);
				dataInstanceXML.setAttribute("description",dataItems[j].description);
				break;
			}
		}
		var requestXML=XMLDoc.createElement("Request");
		requestXML.setAttribute("baseUrl",dataInstances[i].url);
		var parameterXML=XMLDoc.createElement("Parameter");
		parameterXML.setAttribute("key","dataId");
		parameterXML.setAttribute("value",dataInstances[i].dataId);
		parameterXML.setAttribute("description",dataInstances[i].description);
		requestXML.appendChild(parameterXML);
		dataInstanceXML.appendChild(requestXML);
		dataInstancesXML.appendChild(dataInstanceXML);
	}
	computationalSceneXML.appendChild(dataInstancesXML);

	for(var i=0;i<operationInstances.length;i++){
		var operationInstanceXML=XMLDoc.createElement("OperationInstance");
		operationInstanceXML.setAttribute("id",operationInstances[i].uid);
		for(var j=0;j<operations.length;j++){
			if(operations[j].uid==operationInstances[i].uid){
				operationInstanceXML.setAttribute("name",operations[j].name);
				operationInstanceXML.setAttribute("description",operations[j].description);
			}
		}
		var requestXML=XMLDoc.createElement("Request");
		requestXML.setAttribute("baseUrl",operationInstances[i].request.url);
		var parameter1XML=XMLDoc.createElement("Parameter");
		parameter1XML.setAttribute("key","Id");
		parameter1XML.setAttribute("value",operationInstances[i].request.operationId);
		parameter1XML.setAttribute("description",operationInstances[i].request.idDesc);
		requestXML.appendChild(parameter1XML);
		var parameter2XML=XMLDoc.createElement("Parameter");
		parameter2XML.setAttribute("key","Method");
		parameter2XML.setAttribute("value",operationInstances[i].request.methodName);
		parameter2XML.setAttribute("description",operationInstances[i].request.methodDesc);
		requestXML.appendChild(parameter2XML);
		operationInstanceXML.appendChild(requestXML);
		for(var j=0;j<operationInstances[i].dataList.length;j++){
			var dataXML;
			if(operationInstances[i].dataList[j].type=="in"){
				dataXML=XMLDoc.createElement("Input");
			}
			else{
				dataXML=XMLDoc.createElement("Output");
			}
			dataXML.setAttribute("description",operationInstances[i].dataList[j].description);
			dataXML.setAttribute("dataInstance",operationInstances[i].dataList[j].dataId);
			operationInstanceXML.appendChild(dataXML);
		}
		operationInstancesXML.appendChild(operationInstanceXML);
	}
	computationalSceneXML.appendChild(operationInstancesXML);

	// console.log(computationalSceneXML);
	var computationalSceneXMLStr=mxUtils.getXml(computationalSceneXML);
	return computationalSceneXMLStr;
}

/**
 * Set computational scene list from XML in type string.
 */
Graph.prototype.setComputationalScene=function(Str){
	if(Str!=""){
		var dataInstances=[];
		var emptyDataReg=/<DataInstanceCollection\/>/;
		if(!emptyDataReg.test(Str)){
			var DataReg=/<DataInstanceCollection>([\S\s]*)<\/DataInstanceCollection>/;
			var dataInstancesXMLStr=Str.match(DataReg)[0];
			var dataInstancesNode=mxUtils.parseXml(dataInstancesXMLStr).children[0];
			for(var i=0;i<dataInstancesNode.children.length;i++){
				let dataInstance={};
				dataInstance.uid=dataInstancesNode.children[i].getAttribute("id");
				dataInstance.url=dataInstancesNode.children[i].children[0].getAttribute("baseUrl");
				dataInstance.dataId=dataInstancesNode.children[i].children[0].children[0].getAttribute("value");
				dataInstance.description=dataInstancesNode.children[i].children[0].children[0].getAttribute("description");
				dataInstances.push(dataInstance);
			}
		}

		var modelInstances=[];
		var emptyModelReg=/<ModelInstanceCollection\/>/;
		if(!emptyModelReg.test(Str)){
			var ModelReg=/<ModelInstanceCollection>([\S\s]*)<\/ModelInstanceCollection>/;
			var modelInstancesXMLStr=Str.match(ModelReg)[0];
			var modelInstancesNode=mxUtils.parseXml(modelInstancesXMLStr).children[0];
			for(var i=0;i<modelInstancesNode.children.length;i++){
				let modelInstance={};
				modelInstance.uid=modelInstancesNode.children[i].getAttribute("id");
				let request={};
				request.url=modelInstancesNode.children[i].children[0].getAttribute("baseUrl");
				request.modelId=modelInstancesNode.children[i].children[0].children[0].getAttribute("value");
				request.idDesc=modelInstancesNode.children[i].children[0].children[0].getAttribute("description");
				modelInstance.request=request;
				let states=[];
				let statesNode=modelInstancesNode.children[i].children[1];
				for(var j=0;j<statesNode.children.length;j++){
					let stateNode=statesNode.children[j];
					let state={};
					state.id=stateNode.getAttribute("id");
					state.name=stateNode.getAttribute("name");
					let events=[];
					for(var k=0;k<stateNode.children.length;k++){
						let event={};
						event.name=stateNode.children[k].getAttribute("name");
						event.type=stateNode.children[k].getAttribute("type");
						event.dataId=stateNode.children[k].getAttribute("dataInstance");
						events.push(event);
					}
					state.events=events;
					states.push(state);
		...
		var emptyOperationReg=/<OperationInstanceCollection\/>/;
		if(!emptyOperationReg.test(Str)){
			var OperationReg=/<OperationInstanceCollection>([\S\s]*)<\/OperationInstanceCollection>/;
			var operationInstancesXMLStr=Str.match(OperationReg)[0];
			var operationInstancesNode=mxUtils.parseXml(operationInstancesXMLStr).children[0];
			for(var i=0;i<operationInstancesNode.children.length;i++){
				let operationInstance={};
				operationInstance.uid=operationInstancesNode.children[i].getAttribute("id");
				let request={};
				request.url=operationInstancesNode.children[i].children[0].getAttribute("baseUrl");
				request.operationId=operationInstancesNode.children[i].children[0].children[0].getAttribute("value");
				request.idDesc=operationInstancesNode.children[i].children[0].children[0].getAttribute("description");
				request.methodName=operationInstancesNode.children[i].children[0].children[1].getAttribute("value");
				request.methodDesc=operationInstancesNode.children[i].children[0].children[1].getAttribute("description");
				operationInstance.request=request;
				let dataList=[];
				for(var j=1;j<operationInstancesNode.children[i].children.length;j++){
					let data={};
					if(operationInstancesNode.children[i].children[j].nodeName=="Input"){
						data.type="in";
					}
					else{
						data.type="out";
					}
					data.description=operationInstancesNode.children[i].children[j].getAttribute("description");
					data.dataId=operationInstancesNode.children[i].children[j].getAttribute("dataInstance");
					dataList.push(data);
				}
				operationInstance.dataList=dataList;
				operationInstances.push(operationInstance);
			}
		}

		this.computationalScene.dataInstances=dataInstances;
		this.computationalScene.modelInstances=modelInstances;
		this.computationalScene.operationInstances=operationInstances;
		// console.log(this.computationalScene);
	}
}
