...
var OpenNewFile3 = function(editorUi)
{
	var div = document.createElement("div");
    var input = document.createElement("input");
	input.type = "file";
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
        var value = fileStr;
        var graph = editorUi.editor.graph;
		graph.setComputationalScene(value);
        editorUi.hideDialog();
    });
    okBtn.className = 'geBtn gePrimaryBtn';
    okBtn.setAttribute("disabled","disabled");
...
var LogicalTasksReadDialog=function(editorUi){
	...
	var groupID=url.match(reg)[1].replace("ComputationalModel","LogicalModel");
    $.ajax({
        type: "POST",
        url: "/modelBuilder/logical/read"+"?collaborativeId="+groupID,
			...
			if(jsonArray.length<1){
				...
				for(var i=0;i<jsonArray.length;i++) {
					str+='<div style="height:150px;width:400px;border:solid 1px #d5d5d5;margin:20px;">\n' +
						...
						'\t\t<span class="deleteTask" style="float:right;margin:5px;cursor:pointer;" data-projectid="'+jsonArray[i]["taskId"]+'" onclick="deleteTask(this)">X</span>\n'+
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
        if ($('input[name="tasks"]:checked').val()){
			...
			window.mxgraphXMLStr=taskData[$('input[name="tasks"]:checked').val()]["graphXML"];
			window.logicalSceneXMLStr=taskData[$('input[name="tasks"]:checked').val()]["logicalXML"];
			for(let i=0;i<graph.mxgraphList.length;i++){
				if(graph.mxgraphList[i].uid=="basicXML"){
					var doc = mxUtils.parseXml(graph.mxgraphList[i].graphXML);
					var dec = new mxCodec(doc);
 ...
 * Constructs a new read dialog
 */
var ComputationalTasksReadDialog=function(editorUi){
    var dialogContent=document.createElement('div');
    ...
    $.ajax({
        type: "POST",
        url: "/modelBuilder/computational/read"+"?collaborativeId="+groupID,
        success: function (data) {
            var str="";
            var jsonArray=eval(data);//字符串转数组
            taskData=jsonArray;
    ...
    {
        if ($('input[name="tasks"]:checked').val()){
            window.taskID=taskData[$('input[name="tasks"]:checked').val()]["taskId"];
			...
			graph.setMxgraphList(taskData[$('input[name="tasks"]:checked').val()]["graphXML"]);
			graph.setLogicalScene(taskData[$('input[name="tasks"]:checked').val()]["logicalXML"]);
			graph.setComputationalScene(taskData[$('input[name="tasks"]:checked').val()]["computationalXML"]);
 ...
 */
var ComputationalTasksSaveDialog=function(editorUi){
    var div = document.createElement("div");
    div.style.textAlign = "center";
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
		...
		dataJSON["graphXML"]=window.mxgraphXMLStr;
		dataJSON["logicalXML"]=window.logicalSceneXMLStr;
		dataJSON["computationalXML"]=graph.getComputationalSceneXMLStr();
        dataJSON["collaborativeId"]=groupID;
        $.ajax({
            type: "POST",
            url: "/modelBuilder/computational/save",
            data: dataJSON,
            dataType: "text",
            success: function (data) {
                if(data!==""&&data!=="undefined"&&data!=null&&data!="Fail"){
                    alert("Success!");
					window.taskID=data;
					window.taskName=$('#graphNameSave').val();
					window.taskDescription=$('#graphDescriptionSave').val();
					editorUi.hideDialog();
                }
            },
            error: function () {
...
var dataSelectDataServiceDialog=function(editorUi,elt){
	var div =document.createElement("div");
	var h3 = document.createElement("h3");
    h3.innerHTML = "Data List";
    h3.style.textAlign = "center";
	div.appendChild(h3);
	
	var dataListDiv=document.createElement("div");
	dataListDiv.style.width="100%";
	dataListDiv.style.height="220px";
	dataListDiv.style.overflowY="auto";
	var dataListHtml="";
	for(var i=0;i<graph.computationalScene.dataInstances.length;i++){
		var name="";
		var description="";
		for(var j=0;j<graph.logicalScene.dataItems.length;j++){
			if(graph.logicalScene.dataItems[j].uid==graph.computationalScene.dataInstances[i].uid){
				name=graph.logicalScene.dataItems[j].name;
				description=graph.logicalScene.dataItems[j].description;
				break;
			}
		}
		dataListHtml+='<div style="width:90%;border:solid 1px #d5d5d5;margin-top:5px;"><input type="radio" name="datas" value="'+i+'"><span style="font-weight:bold">'+
		name+'</span><div><span>'+description+'</span></div></div>';
	}
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
		if ($('input[name="datas"]:checked').val()){
			var selectedUrl=graph.computationalScene.dataInstances[$('input[name="datas"]:checked').val()].url;
			var td=elt.parentNode;
			for(var j=0;j<td.childNodes.length;j++){
				if(td.childNodes[j]==elt){
					td.childNodes[j-1].value=selectedUrl;
					break;
				}
			}
			$(".dataValue").val(graph.computationalScene.dataInstances[$('input[name="datas"]:checked').val()].dataId);
			$("#dataDescription").val(graph.computationalScene.dataInstances[$('input[name="datas"]:checked').val()].description);
		}
        editorUi.hideDialog();
    });
    okBtn.className = 'geBtn gePrimaryBtn';
...
var modelSelectDataServiceDialog=function(editorUi,elt){
	var div =document.createElement("div");
	var h3 = document.createElement("h3");
    h3.innerHTML = "Data List";
    h3.style.textAlign = "center";
    ...
    {
		if ($('input[name="datas"]:checked').val()){
			var selectedId=graph.computationalScene.dataInstances[$('input[name="datas"]:checked').val()].uid;
			var td=elt.parentNode;
			for(var j=0;j<td.childNodes.length;j++){
				if(td.childNodes[j]==elt){
					td.childNodes[j-1].value=selectedId;
					break;
				}
			}
		}
...
var SetDataServiceDialog = function(editorUi,title,uid)
{
	...
	buttonAdd.style.float="initial";
	buttonAdd.onclick=function(){
		editorUi.showDialog(new dataSelectDataServiceDialog(editorUi,this).container,300,300,true,false);
	}
    BaseUrlPanel.appendChild(buttonAdd);
	...
	inputDesc.type="text";
	inputDesc.className="dataDesc";
	inputDesc.id="dataDescription";
	if(exist){
		inputDesc.value=dataInstances[index].description;
	}
	...
	buttonGroupDiv.style.textAlign="right";
    var cancelBtn = mxUtils.button(mxResources.get('cancel'), function()
    {
        editorUi.hideDialog();
    });
    ...
    if (editorUi.editor.cancelFirst)
    {
        buttonGroupDiv.appendChild(cancelBtn);
    }
    ...
    okBtn.className = 'geBtn';
    buttonGroupDiv.appendChild(okBtn);
...
var SetModelServiceDialog = function(editorUi,title,uid)
{
	...
	for(var i=0;i<modelInstances.length;i++){
		if(modelInstances[i].uid==uid){
			...
			$.ajax({
				...
				// url:modelInstances[i].request.url+"?id="+modelInstances[i].request.modelId,
				success:function(data){
	...
	var buttonSubmitDiv=document.createElement("div");
	buttonSubmitDiv.style.textAlign="right";
	var buttonAdd=document.createElement("button");
	buttonAdd.innerHTML="Connect";
	buttonAdd.style.height="20px";
	buttonAdd.style.margin="10px 10px";
	buttonAdd.style.float="initial";
	buttonAdd.onclick=function(){
		...
		$.ajax({
			...
			// url:$("#modelURLText").val()+"?id="+$(".modelIdValue").val(),
		...
		function addState(modelState){
			...
			for(var j=0;j<modelState.Event.length;j++){
				...
				dataIdSelect.onclick=function(){
					editorUi.showDialog(new modelSelectDataServiceDialog(editorUi,this).container,300,300,true,false);
				};
	...
	buttonSubmitDiv.appendChild(buttonAdd);
	div.appendChild(buttonSubmitDiv);
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
		...
		console.log(graph.computationalScene);
        editorUi.hideDialog();
    });
    okBtn.className = 'geBtn';
    buttonGroupDiv.appendChild(okBtn);
...
var SetOperationServiceDialog = function(editorUi,title,uid)
{
	var graph=editorUi.editor.graph;
	var operationInstances=graph.computationalScene.operationInstances;
	var operationParams=[];
	var exist=0;var index;
	for(var i=0;i<operationInstances.length;i++){
		if(operationInstances[i].uid==uid){
			index=i;exist=1;
			$.ajax({
				url:operationInstances[i].request.url,
				// url:operationInstances[i].request.url+"?id="+operationInstances[i].request.operationId+"&name="+operationInstances[i].request.methodName,
				success:function(data){
					var operationData=JSON.parse(data);
					operationParams=operationData.Params;
					loadTable();
				},
				error:function(){
					alert("please confirm your url!");
    ...
    input.style.width = "400px";
	input.style.margin="10px 0 10px 10px";
	input.id="operationURLText";
	if(exist){
		input.value=operationInstances[index].request.url;
	}
    BaseUrlPanel.appendChild(input);
	...
	tbody.style.minHeight="130px";
	tbody.style.overflowY="auto";
	var requestList=[{"keyClass":"operationId","keyValue":"Operation ID","valueClass":"operationIdValue","descClass":"idDesc"},
	{"keyClass":"methodName","keyValue":"Method Name","valueClass":"methodNameValue","descClass":"methodDesc"}];
	for(var i=0;i<requestList.length;i++){
		var tr=document.createElement("tr");
		var tdKey=document.createElement("td");
		tdKey.style.width="20%";
		...
		inputValue.className=requestList[i].valueClass;
		if(exist){
			inputValue.value=operationInstances[index].request[requestList[i].keyClass];
		}
		tdValue.appendChild(inputValue);
		tr.appendChild(tdValue);
		var tdDesc=document.createElement("td");
		...
		descArea.style.width="98%";
		if(exist){
			descArea.value=operationInstances[index].request[requestList[i].descClass];
		}
		tdDesc.appendChild(descArea);
		tr.appendChild(tdDesc);
		tbody.appendChild(tr);
	...
	buttonAdd.onclick=function(){
		datasDiv.innerHTML="";
		$.ajax({
			url:$("#operationURLText").val(),
			// url:$("#operationURLText").val()+"?id="+$(".operationIdValue").val()+"&name="+$(".methodNameValue").val(),
			// url:"http://172.21.212.85:8899/refactor/method?id=59943559795d260678b04cfd&name=genGrid2d",
			success:function(data){
				var operationData=JSON.parse(data);
				operationParams=operationData.Params;
				var table = document.createElement("table");
				...
				var thead = document.createElement("thead");
				var theadTr = document.createElement("tr");
				theadTr.innerHTML = "</th><th width='20%'>Name</th><th width='10%'>Type</th><th width='30%'>DataId</th><th width='40%'>Description</th>"
				thead.appendChild(theadTr);
				table.appendChild(thead);
				var tbody = document.createElement("tbody");
				tbody.id="dataServiceTable";
				tbody.style.minHeight="130px";
				tbody.style.overflowY="auto";
				for(var i=0;i<operationParams.length;i++){
					var tr=document.createElement("tr");
					var tdName=document.createElement("td");
					tdName.style.width="20%";
					...
					dataName.value=operationParams[i].$.name;//dataName
					dataName.disabled=true;
					tdName.appendChild(dataName);
					tr.appendChild(tdName);
					var tdType=document.createElement("td");
					...
					dataType.style.width="90%";
					dataType.value=operationParams[i].$.type;//dataType
					dataType.disabled=true;
					tdType.appendChild(dataType);
					tr.appendChild(tdType);
					var tdDataId=document.createElement("td");
					tdDataId.style.width="30%";
					var dataIdText=document.createElement("input");
					dataIdText.type="text";
					dataIdText.className="DataId";
					dataIdText.style.width="85%";
					tdDataId.appendChild(dataIdText);
					var dataIdSelect=document.createElement("button");
					dataIdSelect.style.padding="0";
					dataIdSelect.innerHTML="+";
					...
					tr.appendChild(tdDataId);
					var tdDesc=document.createElement("td");
					tdDesc.style.width="40%";
					var dataDesc=document.createElement("input");
					dataDesc.type="text";
					dataDesc.className="dataDesc";
					tdDesc.appendChild(dataDesc);
					tr.appendChild(tdDesc);

					tbody.appendChild(tr);
				}
				table.appendChild(tbody);
				datasDiv.append(table);
			},
			error:function(){
				alert("please confirm your url!");
			}
	...
	datasDiv.style.marginBottom="20px";
	datasDiv.style. minHeight="250px";
	datasDiv.id="operationDatasDiv"
	function loadTable(){
		var table = document.createElement("table");
		table.setAttribute("border",1);
		table.setAttribute("cellspacing",0);
		...
		for(var i=0;i<operationParams.length;i++){
			...
			dataIdText.className="DataId";
			dataIdText.style.width="85%";
			dataIdText.value=operationInstances[index].dataList[i].dataId;
			tdDataId.appendChild(dataIdText);
			var dataIdSelect=document.createElement("button");
			dataIdSelect.style.padding="0";
			dataIdSelect.innerHTML="+";
			...
			dataDesc.type="text";
			dataDesc.className="dataDesc";
			dataDesc.value=operationInstances[index].dataList[i].description;//dataDesc
			tdDesc.appendChild(dataDesc);
			tr.appendChild(tdDesc);
    ...
    {
		var request={};
		request.url=$("#operationURLText").val();
		request.operationId=$(".operationIdValue").val();
		request.idDesc=$(".idDesc").val();
		request.methodName=$(".methodNameValue").val();
		request.methodDesc=$(".methodDesc").val();

		var dataTypeList=[];
		$(".dataType").each(function(){
			dataTypeList.push($(this).val());
		});
		var dataIdList=[];
		$(".DataId").each(function(){
			dataIdList.push($(this).val());
		});
		var dataDescList=[];
		$(".dataDesc").each(function(){
			dataDescList.push($(this).val());
		});
		var dataList=[];
		for(var i=0;i<dataIdList.length;i++){
			var data={};
			data.type=dataTypeList[i];
			data.dataId=dataIdList[i];
			data.description=dataDescList[i];
			dataList.push(data);
		}
		if(exist){
			graph.computationalScene.operationInstances[index].uid=uid;
			graph.computationalScene.operationInstances[index].request=request;
			graph.computationalScene.operationInstances[index].dataList=dataList;
		}else{
			var operationInstance={};
			operationInstance.uid=uid;
			operationInstance.request=request;
			operationInstance.dataList=dataList;
			graph.computationalScene.operationInstances.push(operationInstance);
		}
		console.log(graph.computationalScene.operationInstances);
        editorUi.hideDialog();
    });
    okBtn.className = 'geBtn';
    buttonGroupDiv.appendChild(okBtn);
