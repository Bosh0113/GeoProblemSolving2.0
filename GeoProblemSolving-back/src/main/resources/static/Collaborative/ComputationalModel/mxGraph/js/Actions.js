...
Actions.prototype.init = function()
{
	...
	//ReadTask
    this.addAction('OpenLogicalTask', function()
    {
        var dlg = new LogicalTasksReadDialog(ui);
        ui.showDialog(dlg.container, 500, 600, true, false);
	}, null, null, '').isEnabled = isGraphEnabled;

	this.addAction('OpenComputationalTask', function()
    {
        var dlg = new ComputationalTasksReadDialog(ui);
        ui.showDialog(dlg.container, 500, 600, true, false);
	}, null, null, '').isEnabled = isGraphEnabled;
    ...
    {
        var dlg = new ComputationalTasksSaveDialog(ui);
    ...
    this.addAction('UpdateTask', function()
    {
        if(window.taskID==null||window.taskID=="undefined"||window.taskID==""){
            ...
            $.ajax({
                type: "POST",
                url: "/modelBuilder/computational/update",
                data: dataJSON,
                dataType: "text",
	...
	this.addAction('setOperationService',function () {
        var cell = graph.getSelectionCell();
        ui.showDialog(new SetOperationServiceDialog(ui, cell.value,cell.uid).container,700,520,true,false);
