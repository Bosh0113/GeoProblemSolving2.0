...
Sidebar.prototype.init = function()
{
	...
	this.addStencilPalette('flowchart', 'Flowchart', dir + '/logical.xml',
		';whiteSpace=wrap;html=1;fillColor=#ffffff;strokeColor=#000000;strokeWidth=2');
	this.addBasicInfo("modelitem","Model Item",true,LogicalModel.modelItems);
    this.addBasicInfo("dataitem","Data Item",true,LogicalModel.dataItems);
    this.addBasicInfo("condition","Condition Item",true,LogicalModel.conditionItems);
    this.addBasicInfo("operation","Operation Item",true,LogicalModel.operations);
};
...
{
	var elt = document.createElement('a');
	elt.setAttribute('href', 'javascript' +
		':void(0);');
	...
	elt.style.width = (this.thumbWidth*2 + border) + 'px';
	elt.style.height = (this.thumbHeight*2 + border) + 'px';
	...
	this.createThumb(cells, this.thumbWidth*2, this.thumbHeight*2, elt, title, showLabel, showTitle, width, height);
...
Sidebar.prototype.createDragSource = function(elt, dropHandler, preview, cells, bounds)
{
	...
	dragSource.mouseDown = function(evt)
	{
		...
		{
			var value = document.getElementById("viewPanel").value;
			var bId = this.element.id;
			if(value === "Integration"){
				if((bId!="DataItem")&&(bId!="Operation")){
                    graph.stopEditing();
...
Sidebar.prototype.addBasicInfo = function (id,title,expanded,items) {
	var graph = this.editorUi.editor.graph;
    var elt = this.createTitle(title);
    this.container.appendChild(elt);
            ...
            (function (e) {
                ...
                li.onclick = function () {
					...
					for(let i=0;i<graph.mxgraphList.length;i++){
						if(graph.mxgraphList[i].uid==uid){
    ...
    this.addFoldingHandler(elt, div,null);
...
function changeSelect() {
    ...
    var index = select.childElementCount;
    if(index>1){
        select.innerHTML = "";
