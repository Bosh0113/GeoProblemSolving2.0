    ...
    mxUtils.getAll([bundle, STYLE_PATH + '/default.xml'], function (xhr) {
        ...
        if (WebSocket) {
            ...
            if (url.search(reg) != -1) {
                ...
                var  wsUrl = "wss://"+window.location.host+"/PExploration/LogicalModel/" + groupID;
                if(window.location.port=="8083"){
                    wsUrl = "wss://"+ window.location.hostname+":8083/PExploration/LogicalModel/" + groupID;
                }
                wsMxgraph = new WebSocket(wsUrl);
                // wsMxgraph = new WebSocket("wss://localhost:8081/PExploration/LogicalModel/" + groupID);
                // wsMxgraph = new WebSocket("wss://172.21.212.72:8082/PExploration/LogicalModel/" + groupID);
                // wsMxgraph = new WebSocket("wss://94.191.49.160:8080/PExploration/LogicalModel/" + groupID);
                // wsMxgraph = new WebSocket("wss://172.21.213.185:8080/PExploration/LogicalModel/" + groupID);
            }
        }
        else {
