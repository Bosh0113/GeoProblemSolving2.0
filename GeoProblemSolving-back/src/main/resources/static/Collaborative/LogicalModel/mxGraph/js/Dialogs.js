...
var LogicalTasksReadDialog = function (editorUi) {
        ...
        success: function (data) {
            if (data == "Fail") {
                ...
                for (var i = 0; i < jsonArray.length; i++) {
                    if (jsonArray[i].type === "toolData: Logical_modeling") {
                        ...
                        str += '<div style="height:150px;width:400px;border:solid 1px #d5d5d5;margin:20px;">\n' +
                            ...
                            '\t\t<span style="font-weight:bold">' + jsonArray[i]["name"].substring(0, jsonArray[i]["name"].length - 4) + '</span>\n' +
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function () {
        if ($('input[name="tasks"]:checked').val()) {
            ...
            window.taskName = name.substring(0, name.length - 4);;
            ...
            xhr.onload = function () {
                if (this.status == 200) {
                    ...
                    reader.onload = function (e) {
                        ...
                        var graph = editorUi.editor.graph;
                        graph.setMxgraphList(content.graphXML);
                        graph.setLogicalScene(content.logicalXML);

                        document.getElementById("graphName").value = name.substring(0, name.length - 4);
...
var LogicalTasksSaveDialog = function (editorUi) {
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function () {
        ...
        var fileName = $('#graphNameSave').val() + ".lgm";
        var blobFile = new File([blob], fileName);
        var description = $('#graphDescriptionSave').val();
        var toolInfo = {
            toolName: "Logical modeling",
            toolUrl: "/GeoProblemSolving/Collaborative/LogicalModel/index.html"
        };

        var formData = new FormData();
        formData.append("file", blobFile);
        formData.append("description", description);
        formData.append("type", "toolData: Logical_modeling");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data === "Size over" || data === "Fail" || data === "Offline") {
                    ...
                    alert("Success!");
                    window.taskID = data.uploaded[0].resourceId;
