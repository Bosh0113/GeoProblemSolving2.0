...
@Component
@ServerEndpoint(value = "/ChartsServer/{roomId}", configurator = GetHttpSessionConfigurator.class)
public class ChartsServerSocket {

    private Session session=null;
    private static final Map<String,Map<String,ChartsServerSocket>> rooms =new ConcurrentHashMap<>();
    ...
    public void onOpen(@PathParam("roomId") String roomId, Session session,EndpointConfig config)
    {
        ...
        if (!rooms.containsKey(roomId)){
            Map<String,ChartsServerSocket> room=new ConcurrentHashMap<>();
    ...
    {
        for (Map.Entry<String, ChartsServerSocket> server : rooms.get(roomId).entrySet()) {
